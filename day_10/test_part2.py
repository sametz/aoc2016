...
def test_dispatch():
    ...
    assert not dispatch(
        ("2", "bot", "1", "bot", "0"), receivers, target=TARGET_COMPARISON_TEST
    )
    assert dispatch(("3", "1"), receivers, target=TARGET_COMPARISON_TEST)
    assert receivers["bot"]["1"] == ["3"]
    assert not dispatch(
        ("1", "output", "1", "bot", "0"), receivers, target=TARGET_COMPARISON_TEST
    )
    assert not dispatch(
        ("0", "output", "2", "output", "0"), receivers, target=TARGET_COMPARISON_TEST
    )
